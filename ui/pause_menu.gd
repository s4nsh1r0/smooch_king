...
const ConfirmationDialogScene = preload("res://ui/confirmation_dialogue.tscn")
...
func _input(event: InputEvent) -> void:
	if event is InputEventKey and event.pressed and event.keycode == KEY_ESCAPE:
		# Don't handle escape if dialogs are open
		if confirmation_dialog and confirmation_dialog.visible:
			return
		if load_game_scene and load_game_scene.visible:
			return
		if main_menu_confirmation_dialog and main_menu_confirmation_dialog.visible:
			return
		...
		if is_instance_valid(panel_container):
			...
			printerr("PauseMenu: PanelContainer invalid")
...
func _on_save_button_pressed() -> void:
	if not PlayerData:
		printerr("PauseMenu: PlayerData not found!")
		if UIManager:
			UIManager.show_notification("Error: PlayerData not available")
		return
	
	# Use the new SaveManager
	var result = SaveManager.save_game(PlayerData, false)
	
	if result.success:
		if UIManager:
			UIManager.show_notification(result.message)
		hide_menu()
	elif result.has("needs_confirmation") and result.needs_confirmation:
		# Show confirmation dialog for overwrite
		_show_save_confirmation(result)
	else:
...
func _on_load_button_pressed() -> void:
	...
	# Hide pause menu and show load game scene
	panel_container.visible = false
...
func _on_main_menu_confirmed():
	...
	var main_menu_path = "res://scenes/start_scene.tscn"
...
func _on_load_back_requested():
	# Show pause menu and hide load game scene
	if load_game_scene:
		load_game_scene.visible = false
	panel_container.visible = true
